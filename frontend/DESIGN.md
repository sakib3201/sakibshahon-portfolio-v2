...
- **The Periodical (YouTube):** Video cards carry a cinnabar play-glyph seal (small hanko square, inktext play triangle, slight rotation) at the lower-right of each card's description block — kept clear of the thumbnail — that re-stamps with a faint gold ring on card hover/focus. A `gold-edge` free-note chip (`marginalia`, goldbright, diamond marker) sits under the blurb while the series is free. The empty state is a dashed cinnabar outline seal with a dim play glyph — a stamp waiting for the first video, never a fabricated placeholder.
