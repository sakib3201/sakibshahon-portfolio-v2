...
- **The Journal (about journey):** On `/about`, the journey is a bound book, not a timeline. A lacquer cover (brush "My Journey", cinnabar 誌 seal, gold needle rules inside a gold hairline frame) turns open onto the first of nine `skin-sheet` leaves, and a fin leaf (cinnabar 完 seal, "the story is still being written") closes it. Leaves turn over the spine in 3D (`rotateY 0 → -180°` around `transform-origin: left center`, 0.7s `cubic-bezier(0.22,1,0.36,1)`, `perspective` 2600px, crease shading at the spine edge): the departing leaf's back shows a colophon — a cinnabar year seal and the chapter title on `paperdim` — on the left half, while the next leaf rises from beneath. The read pages pile behind the spine, each shifted a hair down-left (`--depth × 4px`) and stacked a level lower. Leaves are half-width on md+ (the spread reads previous-colophon / current-chapter) and full-width below sm (the turn goes over the book's left edge; the pile lands off-screen, clipped by `overflow-x: clip` on `html`). Flip via page-corner diamond tabs, prev/next leaf buttons with a `LEAF 04 / 11` marginalia counter, ArrowLeft/ArrowRight, or a swipe — one flip at a time (0.72s guard); each chapter change announces through a polite `aria-live` region. The book layout activates only under `html.motion-init.book-enabled` (toggled by the component when motion is allowed): with no JS or under `prefers-reduced-motion` the leaves render as a static, fully readable stack — cover, nine entries, fin — in DOM order, with all controls hidden. **WebGL curl layer:** when the stage approaches the viewport and the engine reports a live first frame, `html.webgl-journal` upgrades the turn — the DOM leaves become `visibility: hidden` layout placeholders (the stage keeps its height, content stays the source of truth) and a three.js canvas fills the slot. The leaf bends around a moving fold line (bend radius ≈ 18px world, `α = min(d/r, θ)`, `θ: 0 → π` over the same 0.7s curve, then a 140ms un-curl settle onto the pile — the "soft drop"), with a gold-edge catch and crease darkening baked into per-vertex colors, numeric normals, a warm gold key + cool-dim fill on black ambient (no colored glows — gold light only), and a hair of pointer parallax (`pointer: fine` only). The fold is lit, textured paper bent into the binding; the read pages rest as a flat pile behind the spine with two dark depth slabs. **Fallback ladder:** WebGL init or texture-snapshot failure removes `webgl-journal` and the CSS book is untouched — the ladder is WebGL curl → CSS pivot → static stack; a blank journal never ships.
- **The Periodical (YouTube):** Video cards are `lacquer-raised` sheets with the thumbnail full-bleed on top and the title and description below — no play seal on the cards, so the thumbnails stay clean. A `gold-edge` free-note chip (`marginalia`, goldbright, diamond marker) sits under the blurb while the series is free. The empty state is a dashed cinnabar outline seal with a dim play glyph — a stamp waiting for the first video, never a fabricated placeholder.
...
- **2026-08 ink-detail batch (#39–44):** impact stat count-up (1.2s ease-out, once on view, per-card 60ms stagger ≤ 240ms cap, divider draws after settle), ledger page unroll (0.7s clip-path) then badge stamp (0.35s), wax-seal stamp (0.4s) + gold bloom (0.8s), screenshot ink-wash colorize (0.5s) — all one-shots or hover states, gated off-screen and disabled under reduced motion like every ambient loop.
- **2026-08 journal batch:** the about journey becomes a bound journal — cover and fin leaves, page-turn (0.7s, the focal band), back-face colophon colophons, corner tabs / leaf buttons / keys / swipe, one-flip-at-a-time guard, static stacked fallback under reduced motion or no JS.
- **2026-08 journal WebGL batch:** the CSS pivot is layered under a three.js curl turn — same 0.7s `cubic-bezier(0.22,1,0.36,1)` fold (bend radius ≈ 18px, `θ: 0 → π`) with a 140ms un-curl settle, gold-edge catch + crease shading in vertex colors, per-frame numeric normals, warm gold key + cool-dim fill (no glows), pointer parallax (`pointer: fine` only) — gated behind `html.webgl-journal` with the CSS pivot as the fallback; never starts under reduced motion or without motion-init, and the chunk loads lazily only when the stage nears the viewport.
...
Under `prefers-reduced-motion: reduce`, the site must be fully static and fully readable: all durations and delays collapse to ~0.01ms, the `motion-init` class is removed so every `[data-reveal]` shows at full opacity, parallax transforms are cleared, tilt listeners are never attached, the hero entrance never plays, the sakura petals and seal guides are never created, and the journal section renders as a static stack of pages (no flip, all chapters readable) — the WebGL engine never starts under reduced motion or with no JS, and the `three` chunk is never loaded. Nothing that hides content is allowed to depend on animation completing.
